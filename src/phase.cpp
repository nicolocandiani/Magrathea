...
PhaseDgm::PhaseDgm (string Comp_name, EOS *(*f) (double, double), int k,
                    EOS **phase_name, double *start_pressure)
{
  Comp_type = Comp_name;
  phase_lowP = f;
  n = k;
...
void
PhaseDgm::set_phase_highP (int k, double *start_pressure, EOS **phase_name)
// start pressure is an array with dimension k-1.  The first phase will replace
// the one with the same name in the phase diagram.
{
  if (n > 1)
    delete[] start_P;
  if (n > 0)
...
EOS *
PhaseDgm::find_phase (double P, double T)
...
double
checkphase (double P, void *params)
{
  ...
  if (P < Pl || P > Pu)
    {
      cout << "Pressure " << P / 1E10 << " is outside the bound between " << Pl
           << " and " << Pu
           << " when solving the pressure of phase boundary for component "
           << cmpn->getname () << " from phase " << p->Phase->getEOS ()
           << endl;
  ...
  if (P == Pu) // In case the output Tt is very close but not exact Tu or Tl.
    ...
    Tt = (p->Tl * (Pu - P) + p->Tu * (P - Pl))
         / (Pu - Pl); // Linear interpolate temperature at P
...
EOS *
PhaseDgm::find_phase_boundary (double Pl, double Pu, double Tl, double Tu,
                               bool inward, double &Po, double &To,
                               double &rhoo, double &Pn, double &Tn,
                               double &rhon)
// Used when integrate adiabatic profile across the phase boundary.  Given the
// pressure in cgs, integration direction, the lower and upper pressure and
// temperature limit (at previous and next integral step depends on the
// integration direction).  Return the pressure, temperature, density at old
// (o) phase boundary and new (n) phase boundary.
{
  EOS *new_phase;
  double P1, P2;
  ...
  if (status == GSL_EINVAL)
    {
      cout << "Error: The phase transition boundary is not between the lower "
              "pressure bound "
           << Pl << " and upper pressure bound " << Pu << ", low temperature "
           << Tl << " and high temperature " << Tu
           << " when find the phase boundary of component " << Comp_type
           << " from phase " << old_phase->getEOS () << " to new phase ";
    ...
    {
      cout << "Error: Failed to set up the phase boundary of component "
           << Comp_type << " from phase " << old_phase->getEOS () << endl;
      gsl_root_fsolver_free (s);
      Po = numeric_limits<double>::quiet_NaN ();
      To = numeric_limits<double>::quiet_NaN ();
      rhoo = numeric_limits<double>::quiet_NaN ();
  ...
  if (status == GSL_CONTINUE)
    {
      cout << "Error: Can't find the pressure of the phase boundary from "
           << find_phase (Pl, Tl)->getEOS () << " and "
           << find_phase (Pu, Tu)->getEOS () << " at pressure " << Pl / 1E10
           << " GPa within maximum interation " << max_iter << endl;
      gsl_root_fsolver_free (s);
      Po = numeric_limits<double>::quiet_NaN ();
      To = numeric_limits<double>::quiet_NaN ();
      rhoo = numeric_limits<double>::quiet_NaN ();
  ...
  Tn = (Tl * (Pu - Pn) + Tu * (Pn - Pl))
       / (Pu - Pl); // Linear interpolate temperature at P
  To = (Tl * (Pu - Po) + Tu * (Po - Pl))
       / (Pu - Pl); // Linear interpolate temperature at P
...
double
dunaeva_phase_curve (double P, double a, double b, double c, double d,
                     double e)
{
  ...
  if (P <= 0)
    {
      cout << "Error: P = " << P
           << " bar. Dunaeva phase curve can't handle negative pressure."
           << endl;
      return numeric_limits<double>::quiet_NaN ();
    }
...
EOS *
find_water_phase (double P, double T)
// input P in cgs
{
  ...
  if (P < 0.208566) // liquid water or Ice Ih (Dunaeva et al. 2010)
    {
      ...
      if (!gsl_finite (Tt1))
        {
          cout << "Error: Can't find the phase of H2O at P=" << P
               << " GPa and T=" << T << " K." << endl;
          return NULL;
        }
      if (T > Tt1)
    ...
    {
      if (T > 1200) // A dummy melting curve to avoid ice VII EOS extrapolated
  ...
  else if (P < 30.9) // liquid water or Ice VII'.
    {
      if (T > 1200) // A dummy melting curve to avoid Ice VII EOS extrapolated
        ...
        // return IceVIIp;         //Region of possible transitional Ice VII'
        // reported in Grande not used in default
        return IceVII_Bezacier;
    }
  else // liquid water or Ice X. The melting curve is so uncertain. Assuming
       // all ice above 30.9 GPa.
    return IceX;
}
...
EOS *
find_Fe_phase (double P, double T)
{
  if (P <= 0 || T <= 0)
    {
      return NULL;
  ...
  if (T > 12.8 * P + 2424
      && T > 13.7 * P
                 + 2328) // melting curve from Dorogokupets et al. 2017,
                         // Scientific Reports. fcc and hcp Fe melting curve.
    return Fe_liquid;
  else
    return Fe_hcp; // use hcp Iron for all regions.

  // Alternative Core Phase Diagram with fcc and bcc iron
  // if(T>575+18.7*P+0.213*pow(P,2)-0.000817*pow(P,3) && P<98.5) // Anzellini
  // et al. 2013
  //{
...
EOS *
find_Si_phase (double P, double T)
{
  if (P <= 0 || T <= 0)
    {
      return NULL;
  ...
  P /= 1E10; // convert microbar to GPa
  // Default Mantle
  if (P > 112.5 + 7E-3 * T) // Phase transfer curve from Ono & Oganov 2005,
                            // Earth Planet. Sci. Lett. 236, 914
    return Si_PPv_Sakai;
  else if (T > 1830 * pow (1 + P / 4.6, 0.33)) // Melting curve from Belonoshko
                                               // et al. 2005 Eq. 2
  ...
  // if(P > 112.5 + 7E-3*T)      // Phase transfer curve from Ono & Oganov
  // 2005, Earth Planet. Sci. Lett. 236, 914
  //  return Si_PPv_Sakai;
  // else if (T > 1830*pow(1+P/4.6, 0.33)) // Melting curve from Belonoshko et
  // al. 2005 Eq. 2
  //  return Si_Liquid_Wolf;
  // else if (P > 24.3+(-2.12E-4*T)+(-3.49E-7*pow(T, 2))) // Dorogokupets et
  // al. 2015
  //    return Pv_Doro;
...
EOS *
find_gas_phase (double P, double T)

{
  ...
  // Isothermal gas for P less than 100 bar, the radiative/convective boundary
  // (Nixon & Madhusudhan 2021). Adiabatic idea gas for P > 100 bar.
  else if (P < 1E8)
    return Gas_iso;
  else
    return Gas;
...
PhaseDgm water ("water", find_water_phase);
PhaseDgm Fe ("core", find_Fe_phase);
PhaseDgm Si ("mantle", find_Si_phase);
PhaseDgm atm ("atm", find_gas_phase);

EOS *
find_phase (double m, double MC, double MM, double MW, double MG, double P,
            double T, bool inward)
// given the accumulated mass (in g), P (in cgs) and T, return the
// corresponding phase
{
  if (inward)
    {
      if (m < MC * ME) // iron core
        return Fe.find_phase (P, T);
...
EOS *
find_phase (double m, vector<PhaseDgm> &Comp, vector<double> M, double P,
            double T, bool inward)
{
  if (Comp.size () != M.size ())
    {
